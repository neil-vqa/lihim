    ...
    try:
        ...
        if not key_file.exists():
            ...
            raise FileNotFoundError(
                errno.ENOENT, os.strerror(errno.ENOENT), "Key not found."
            )
    except Exception as e:
        raise e
...
def enter_user(username: str, password: str, key_path: str, key_name: str) -> None:
    ...
    auth = {"LIHIM_USER": username, "LIHIM_PASSWORD": password, "LIHIM_KEY": key}
...
def create_group(name: str, current_user: User) -> None:
    try:
        new_group = Group(name=name, user=current_user)
        new_group.save()
    except Exception as e:
        raise e
...
def check_key_value(key: str, current_user: User) -> List[Tuple[str, str, str]]:
    ...
    key_val = [
        (
            pair.key_string,
            use_key(key_file, decrypt_text=pair.value_string),
            pair.group.name,
        )
        for pair in pairs
        if pair.key_string == key
    ]
    return key_val
